...
{enum JointIndices
    ...
    public class BoneController : MonoBehaviour
    {
        ...
        private List<GameObject> m_AnchorsList = new List<GameObject>();
        ...
        public void InitializeSkeletonJoints()
        {
            ...
            for (var i = 0; i < k_NumSkeletonJoints; i++)
            {
                var anchor = Instantiate(m_AnchorPrefab);
                anchor.transform.position = m_BoneMapping[i].transform.position;
                anchor.transform.rotation = m_BoneMapping[i].transform.rotation;
        ...
        public void ApplyBodyPose(ARHumanBody body)
        {
            ...
            {
                XRHumanBodyJoint joint = joints[i];
                m_AnchorsList[i].transform.parent = body.transform;
                m_AnchorsList[i].transform.position = body.pose.position + joint.anchorPose.position;
                m_AnchorsList[i].transform.rotation = body.pose.rotation * joint.anchorPose.rotation;
            }
        }
